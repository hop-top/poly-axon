...
var happyPathOverrides = map[string]happyPathOverride{
	axon.HostCopilot: {
...
func TestConformance_CLIIsKnown(t *testing.T) {
	...
	for _, a := range allAdapters {
		t.Run(string(a.CLI()), func(t *testing.T) {
			if _, ok := axon.Get(a.CLI()); !ok {
				t.Errorf("adapter CLI %q is not registered in axon.Get", a.CLI())
			}
		})
	}
}
