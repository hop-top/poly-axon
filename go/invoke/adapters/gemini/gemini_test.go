...
func TestAdapterCLI(t *testing.T) {
	t.Parallel()
	if got := New().CLI(); got != axon.HostGemini {
		t.Errorf("CLI() = %q, want %q", got, axon.HostGemini)
	}
}
...
func TestBuildModeRunUsesDashP(t *testing.T) {
	...
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGemini, Mode: invoke.ModeRun, Prompt: "hi",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildModeResumeWithSession(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGemini, Mode: invoke.ModeResume, SessionID: "abc",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildContinueResumesLatest(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGemini, Mode: invoke.ModeResume, Continue: true,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildResumeRequiresIDOrContinue(t *testing.T) {
	...
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGemini, Mode: invoke.ModeResume,
	})
	if err == nil {
...
func TestBuildForkRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGemini, Mode: invoke.ModeRun, Fork: true,
	})
	if err == nil {
...
func TestBuildAgentRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGemini, Mode: invoke.ModeRun, Agent: "reviewer",
	})
	if err == nil {
...
func TestBuildApprovalModes(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(string(tc.a), func(t *testing.T) {
			spec, _, err := New().Build(invoke.Invocation{
				CLI: axon.HostGemini, Mode: invoke.ModeRun, Approval: tc.a,
			})
			if err != nil {
				t.Fatalf("err = %v", err)
			}
...
func TestBuildApprovalNeverRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGemini, Mode: invoke.ModeRun, Approval: invoke.ApprovalNever,
	})
	if err == nil {
...
func TestBuildApprovalAutoAllRequiresOptIn(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGemini, Mode: invoke.ModeRun, Approval: invoke.ApprovalAutoAll,
	})
	if err == nil {
...
func TestBuildApprovalAutoAllOptIn(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGemini, Mode: invoke.ModeRun,
...
func TestBuildOutputJSON(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGemini, Mode: invoke.ModeRun, Output: invoke.OutputJSON,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
