...
func TestAdapterCLI(t *testing.T) {
	t.Parallel()
	if got := New().CLI(); got != axon.HostOpencode {
		t.Errorf("CLI() = %q, want %q", got, axon.HostOpencode)
	}
}
...
func TestBuildModeRunSubcommand(t *testing.T) {
	...
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeRun, Prompt: "hi",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildModeResumeWithSession(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeResume, SessionID: "abc",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildContinue(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeResume, Continue: true,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildFork(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeResume, SessionID: "x", Fork: true,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildForkRequiresResume(t *testing.T) {
	...
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeRun, Fork: true,
	})
	if err == nil {
...
func TestBuildResumeNeedsIDOrContinue(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeResume,
	})
	if err == nil {
...
func TestBuildOutputJSONIsShim(t *testing.T) {
	...
	spec, ds, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeRun, Output: invoke.OutputJSON,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildOutputStreamJSONNative(t *testing.T) {
	t.Parallel()
	spec, ds, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeRun, Output: invoke.OutputStreamJSON,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildSandboxReadOnlyRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeRun, Sandbox: invoke.SandboxReadOnly,
	})
	if err == nil {
...
func TestBuildSandboxDangerNeedsOptIn(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeRun, Sandbox: invoke.SandboxDangerFullAccess,
	})
	if err == nil {
...
func TestBuildApprovalAutoEditRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeRun, Approval: invoke.ApprovalAutoEdit,
	})
	if err == nil {
...
func TestBuildApprovalAutoAllOptIn(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeRun, Approval: invoke.ApprovalAutoAll,
...
func TestBuildSandboxAndApprovalDontDoublyEmitDangerFlag(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeRun,
...
func TestBuildAddDirsS2Walk(t *testing.T) {
	...
	for _, p := range []string{"a.go", "sub/b.go", "sub/c.go"} {
		...
		_ = os.MkdirAll(filepath.Dir(full), 0o755) //nolint:gosec // test-only fixture tree under t.TempDir()
		_ = os.WriteFile(full, []byte("x"), 0o644) //nolint:gosec // test-only fixture file under t.TempDir()
	}
	spec, ds, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeRun,
		AddDirs: []string{root},
...
func TestBuildAddDirsS2OverflowRefused(t *testing.T) {
	...
	root := t.TempDir()
	for i := range 5 {
		_ = os.WriteFile(filepath.Join(root, string(rune('a'+i))+".txt"), []byte("x"), 0o644) //nolint:gosec // test-only fixture file under t.TempDir()
	}
...
func TestBuildAddDirsAndFilesNoDoubleListing(t *testing.T) {
	...
	for _, p := range []string{a, b} {
		_ = os.WriteFile(p, []byte("x"), 0o644) //nolint:gosec // test-only fixture file under t.TempDir()
	}
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeRun,
		Files:   []string{a},
...
func TestBuildCWDViaDir(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostOpencode, Mode: invoke.ModeRun, CWD: "/repo",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
