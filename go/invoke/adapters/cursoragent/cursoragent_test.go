...
func TestAdapterCLI(t *testing.T) {
	t.Parallel()
	if got := New().CLI(); got != axon.HostCursorAgent {
		t.Errorf("CLI() = %q, want %q", got, axon.HostCursorAgent)
	}
}
...
func TestBuildModeRunPrint(t *testing.T) {
	...
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCursorAgent, Mode: invoke.ModeRun, Prompt: "hi",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildResumeWithID(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCursorAgent, Mode: invoke.ModeResume, SessionID: "abc",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildContinueUsesResumeSubcommand(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCursorAgent, Mode: invoke.ModeResume, Continue: true,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildResumeNeedsIDOrContinue(t *testing.T) {
	...
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCursorAgent, Mode: invoke.ModeResume,
	})
	if err == nil {
...
func TestBuildForkRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCursorAgent, Mode: invoke.ModeRun, Fork: true,
	})
	if err == nil {
...
func TestBuildAgentRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCursorAgent, Mode: invoke.ModeRun, Agent: "x",
	})
	if err == nil {
...
func TestBuildOutputFormats(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(string(tc.out), func(t *testing.T) {
			spec, _, err := New().Build(invoke.Invocation{
				CLI: axon.HostCursorAgent, Mode: invoke.ModeRun, Output: tc.out,
			})
			if err != nil {
				t.Fatalf("err = %v", err)
			}
...
func TestBuildOutputJSONInteractiveRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCursorAgent, Mode: invoke.ModeInteractive, Output: invoke.OutputJSON,
	})
	if err == nil {
...
func TestBuildSandboxTiersRefused(t *testing.T) {
	...
	for _, s := range []invoke.SandboxMode{invoke.SandboxReadOnly, invoke.SandboxWorkspaceWrite} {
		_, _, err := New().Build(invoke.Invocation{
			CLI: axon.HostCursorAgent, Mode: invoke.ModeRun, Sandbox: s,
		})
		if err == nil {
...
func TestBuildApprovalAutoEditRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCursorAgent, Mode: invoke.ModeRun, Approval: invoke.ApprovalAutoEdit,
	})
	if err == nil {
...
func TestBuildApprovalAutoAllOptIn(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCursorAgent, Mode: invoke.ModeRun,
