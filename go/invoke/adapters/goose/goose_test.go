...
func TestAdapterCLI(t *testing.T) {
	t.Parallel()
	if got := New().CLI(); got != axon.HostGoose {
		t.Errorf("CLI() = %q, want %q", got, axon.HostGoose)
	}
}
...
func TestBuildModeRunSubcommand(t *testing.T) {
	...
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGoose, Mode: invoke.ModeRun, Prompt: "hi",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildModeInteractiveUsesSession(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGoose, Mode: invoke.ModeInteractive,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildResumeWithIDUsesRunResume(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGoose, Mode: invoke.ModeResume, SessionID: "abc",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildContinue(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGoose, Mode: invoke.ModeResume, Continue: true,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildForkUsesSessionResumeFork(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGoose, Mode: invoke.ModeResume, SessionID: "abc", Fork: true,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildForkRequiresResume(t *testing.T) {
	...
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGoose, Mode: invoke.ModeRun, Fork: true,
	})
	if err == nil {
...
func TestBuildAgentShimsToRecipe(t *testing.T) {
	...
	spec, ds, err := New().Build(invoke.Invocation{
		CLI: axon.HostGoose, Mode: invoke.ModeRun, Agent: "code-review",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildSandboxAndApprovalRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGoose, Mode: invoke.ModeRun, Sandbox: invoke.SandboxReadOnly,
	})
	if err == nil {
		t.Error("expected error: goose has no per-invocation sandbox")
	}
	_, _, err = New().Build(invoke.Invocation{
		CLI: axon.HostGoose, Mode: invoke.ModeRun, Approval: invoke.ApprovalAutoEdit,
	})
	if err == nil {
...
func TestBuildOutputFormats(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(string(tc.out), func(t *testing.T) {
			spec, _, err := New().Build(invoke.Invocation{
				CLI: axon.HostGoose, Mode: invoke.ModeRun, Output: tc.out,
			})
			if err != nil {
				t.Fatalf("err = %v", err)
			}
...
func TestBuildConfigKeys(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostGoose, Mode: invoke.ModeRun,
