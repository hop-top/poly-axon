...
func TestAdapterCLI(t *testing.T) {
	t.Parallel()
	if got := New().CLI(); got != axon.HostKimi {
		t.Errorf("CLI() = %q, want %q", got, axon.HostKimi)
	}
}
...
func TestBuildModeRunUsesPrint(t *testing.T) {
	...
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostKimi, Mode: invoke.ModeRun, Prompt: "hi",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildResumeWithID(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostKimi, Mode: invoke.ModeResume, SessionID: "abc",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildContinue(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostKimi, Mode: invoke.ModeResume, Continue: true,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildForkRefused(t *testing.T) {
	...
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostKimi, Mode: invoke.ModeRun, Fork: true,
	})
	if err == nil {
...
func TestBuildApprovalPlanNative(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostKimi, Mode: invoke.ModeRun, Approval: invoke.ApprovalPlan,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildApprovalAutoEditRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostKimi, Mode: invoke.ModeRun, Approval: invoke.ApprovalAutoEdit,
	})
	if err == nil {
...
func TestBuildApprovalAutoAllOptIn(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostKimi, Mode: invoke.ModeRun, Approval: invoke.ApprovalAutoAll,
...
func TestBuildSandboxAllRefused(t *testing.T) {
	...
	for _, s := range []invoke.SandboxMode{invoke.SandboxReadOnly, invoke.SandboxWorkspaceWrite, invoke.SandboxDangerFullAccess} {
		_, _, err := New().Build(invoke.Invocation{
			CLI: axon.HostKimi, Mode: invoke.ModeRun, Sandbox: s,
		})
		if err == nil {
...
func TestBuildOutputJSONShim(t *testing.T) {
	...
	spec, ds, err := New().Build(invoke.Invocation{
		CLI: axon.HostKimi, Mode: invoke.ModeRun, Output: invoke.OutputJSON,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildOutputStreamJSONNative(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostKimi, Mode: invoke.ModeRun, Output: invoke.OutputStreamJSON,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildAgent(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostKimi, Mode: invoke.ModeRun, Agent: "okabe",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildCWDUsesWorkDir(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostKimi, Mode: invoke.ModeRun, CWD: "/repo",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildAddDirsRepeatable(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostKimi, Mode: invoke.ModeRun,
		AddDirs: []string{"/a", "/b"},
	})
	if err != nil {
		t.Fatalf("err = %v", err)
