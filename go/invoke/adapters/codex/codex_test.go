...
func TestAdapterCLI(t *testing.T) {
	t.Parallel()
	if got := New().CLI(); got != axon.HostCodex {
		t.Errorf("CLI() = %q, want %q", got, axon.HostCodex)
	}
}
...
func TestBuildModeRunSubcommand(t *testing.T) {
	...
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeRun, Prompt: "hi",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildModeInteractiveNoSubcommand(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeInteractive, Prompt: "hi",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildModeResumeDefaultIsInteractive(t *testing.T) {
	...
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeResume, SessionID: "abc",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildContinueAddsLast(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeResume, Continue: true,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildForkUsesForkSubcommand(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeResume, SessionID: "x", Fork: true,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildForkRequiresResume(t *testing.T) {
	...
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeRun, Fork: true,
	})
	if err == nil {
...
func TestBuildResumeRequiresIDOrContinue(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeResume,
	})
	if err == nil {
...
func TestBuildSandboxTiers(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(string(tc.s), func(t *testing.T) {
			spec, _, err := New().Build(invoke.Invocation{
				CLI: axon.HostCodex, Mode: invoke.ModeRun, Sandbox: tc.s,
			})
			if err != nil {
				t.Fatalf("err = %v", err)
			}
...
func TestBuildSandboxDangerNeedsOptIn(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeRun,
...
func TestBuildApprovalAutoEditRefused(t *testing.T) {
	...
	_, ds, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeRun, Approval: invoke.ApprovalAutoEdit,
	})
	if err == nil {
...
func TestBuildApprovalPlanShimsViaSandboxAndNever(t *testing.T) {
	...
	spec, ds, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeRun, Approval: invoke.ApprovalPlan,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildOutputJSONRequiresPath(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeRun, Output: invoke.OutputJSON,
	})
	if err == nil {
...
func TestBuildOutputStreamJSON(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeRun, Output: invoke.OutputStreamJSON,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildCWDEmitsBothFlagAndDir(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeRun, CWD: "/repo",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildAgentRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCodex, Mode: invoke.ModeRun, Agent: "x",
	})
	if err == nil {
