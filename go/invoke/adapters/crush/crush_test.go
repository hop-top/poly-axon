...
func TestAdapterCLI(t *testing.T) {
	t.Parallel()
	if got := New().CLI(); got != axon.HostCrush {
		t.Errorf("CLI() = %q, want %q", got, axon.HostCrush)
	}
}
...
func TestBuildModeRunSubcommand(t *testing.T) {
	...
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCrush, Mode: invoke.ModeRun, Prompt: "hi",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildResume(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCrush, Mode: invoke.ModeResume, SessionID: "abc",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildContinue(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCrush, Mode: invoke.ModeResume, Continue: true,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildOutputJSONRefused(t *testing.T) {
	...
	for _, out := range []invoke.OutputFormat{invoke.OutputJSON, invoke.OutputStreamJSON} {
		_, _, err := New().Build(invoke.Invocation{
			CLI: axon.HostCrush, Mode: invoke.ModeRun, Output: out,
		})
		if err == nil {
...
func TestBuildForkAndAgentRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCrush, Mode: invoke.ModeRun, Fork: true,
	})
	if err == nil {
		t.Error("expected error: crush has no fork")
	}
	_, _, err = New().Build(invoke.Invocation{
		CLI: axon.HostCrush, Mode: invoke.ModeRun, Agent: "x",
	})
	if err == nil {
...
func TestBuildApprovalAutoEditRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCrush, Mode: invoke.ModeRun, Approval: invoke.ApprovalAutoEdit,
	})
	if err == nil {
...
func TestBuildApprovalAutoAllOptIn(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCrush, Mode: invoke.ModeRun,
...
func TestBuildCWDViaCwd(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCrush, Mode: invoke.ModeRun, CWD: "/repo",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
