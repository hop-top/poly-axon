...
func TestAdapterCLI(t *testing.T) {
	t.Parallel()
	if got := New().CLI(); got != axon.HostQwen {
		t.Errorf("CLI() = %q, want %q", got, axon.HostQwen)
	}
}
...
func TestBuildModeRunPositional(t *testing.T) {
	...
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostQwen, Mode: invoke.ModeRun, Prompt: "hi",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildResumeWithID(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostQwen, Mode: invoke.ModeResume, SessionID: "abc",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildContinue(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostQwen, Mode: invoke.ModeResume, Continue: true,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildResumeNeedsIDOrContinue(t *testing.T) {
	...
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostQwen, Mode: invoke.ModeResume,
	})
	if err == nil {
...
func TestBuildForkAndAgentRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostQwen, Mode: invoke.ModeRun, Fork: true,
	})
	if err == nil {
		t.Error("expected error: qwen has no fork")
	}
	_, _, err = New().Build(invoke.Invocation{
		CLI: axon.HostQwen, Mode: invoke.ModeRun, Agent: "x",
	})
	if err == nil {
...
func TestBuildApprovalModes(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(string(tc.a), func(t *testing.T) {
			spec, _, err := New().Build(invoke.Invocation{
				CLI: axon.HostQwen, Mode: invoke.ModeRun, Approval: tc.a,
			})
			if err != nil {
				t.Fatalf("err = %v", err)
			}
...
func TestBuildApprovalNeverRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostQwen, Mode: invoke.ModeRun, Approval: invoke.ApprovalNever,
	})
	if err == nil {
...
func TestBuildApprovalAutoAllOptIn(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostQwen, Mode: invoke.ModeRun,
...
func TestBuildSandboxBoolean(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostQwen, Mode: invoke.ModeRun, Sandbox: invoke.SandboxWorkspaceWrite,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildSandboxDangerNeedsOptIn(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostQwen, Mode: invoke.ModeRun, Sandbox: invoke.SandboxDangerFullAccess,
	})
	if err == nil {
...
func TestBuildOutputFormats(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(string(tc.out), func(t *testing.T) {
			spec, _, err := New().Build(invoke.Invocation{
				CLI: axon.HostQwen, Mode: invoke.ModeRun, Output: tc.out,
			})
			if err != nil {
				t.Fatalf("err = %v", err)
			}
...
func TestBuildAddDirsRepeatable(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostQwen, Mode: invoke.ModeRun, AddDirs: []string{"/a", "/b"},
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
