...
Re-run `claude --help` and update this section if the surface changes; if the diff touches a flag below, update the matrix in `invoke/README.md` (auto-generated from `Mappings()`).

## Mapping summary

Source of truth: `Mappings()` in `mappings.go`. The package-level
`invoke/README.md` parity table is built from that slice.
