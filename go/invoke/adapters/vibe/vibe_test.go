...
func TestAdapterCLI(t *testing.T) {
	t.Parallel()
	if got := New().CLI(); got != axon.HostVibe {
		t.Errorf("CLI() = %q, want %q", got, axon.HostVibe)
	}
}
...
func TestBuildModeRunUsesP(t *testing.T) {
	...
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostVibe, Mode: invoke.ModeRun, Prompt: "hi",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildResume(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostVibe, Mode: invoke.ModeResume, SessionID: "abc",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildContinue(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostVibe, Mode: invoke.ModeResume, Continue: true,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildModelRefused(t *testing.T) {
	...
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostVibe, Mode: invoke.ModeRun, Model: "x",
	})
	if err == nil {
...
func TestBuildForkRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostVibe, Mode: invoke.ModeRun, Fork: true,
	})
	if err == nil {
	...
	for _, tc := range cases {
		t.Run(string(tc.a), func(t *testing.T) {
			spec, ds, err := New().Build(invoke.Invocation{
				CLI: axon.HostVibe, Mode: invoke.ModeRun, Approval: tc.a,
			})
			if err != nil {
				t.Fatalf("err = %v", err)
			}
...
func TestBuildAgentAndApprovalConflictRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostVibe, Mode: invoke.ModeRun,
...
func TestBuildAgentWithoutApproval(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostVibe, Mode: invoke.ModeRun, Agent: "custom",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildOutputFormats(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(string(tc.out), func(t *testing.T) {
			spec, _, err := New().Build(invoke.Invocation{
				CLI: axon.HostVibe, Mode: invoke.ModeRun, Output: tc.out,
			})
			if err != nil {
				t.Fatalf("err = %v", err)
			}
...
func TestBuildSandboxRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostVibe, Mode: invoke.ModeRun, Sandbox: invoke.SandboxReadOnly,
	})
	if err == nil {
...
func TestBuildCWDViaWorkdir(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostVibe, Mode: invoke.ModeRun, CWD: "/repo",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
