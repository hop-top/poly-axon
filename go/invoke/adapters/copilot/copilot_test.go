...
func TestAdapterCLI(t *testing.T) {
	t.Parallel()
	if got := New().CLI(); got != axon.HostCopilot {
		t.Errorf("CLI() = %q, want %q", got, axon.HostCopilot)
	}
}
...
func TestBuildModeRunUsesDashP(t *testing.T) {
	...
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCopilot, Mode: invoke.ModeRun, Prompt: "hi",
...
func TestBuildModeInteractiveUsesDashI(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCopilot, Mode: invoke.ModeInteractive, Prompt: "hi",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildResumeWithIDEqualsForm(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCopilot, Mode: invoke.ModeResume, SessionID: "abc",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildContinue(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCopilot, Mode: invoke.ModeResume, Continue: true,
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
...
func TestBuildResumeNeedsIDOrContinue(t *testing.T) {
	...
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCopilot, Mode: invoke.ModeResume,
	})
	if err == nil {
...
func TestBuildForkRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCopilot, Mode: invoke.ModeRun, Fork: true,
	})
	if err == nil {
...
func TestBuildOutputJSONIsShim(t *testing.T) {
	...
	spec, ds, err := New().Build(invoke.Invocation{
		CLI: axon.HostCopilot, Mode: invoke.ModeRun, Output: invoke.OutputJSON,
		Approval: invoke.ApprovalAutoAll,
		Config:   map[string]string{"uxp.allow_dangerous": "true"},
	})
	if err != nil {
...
func TestBuildSandboxTiersRefused(t *testing.T) {
	...
	for _, s := range []invoke.SandboxMode{invoke.SandboxReadOnly, invoke.SandboxWorkspaceWrite} {
		_, _, err := New().Build(invoke.Invocation{
			CLI: axon.HostCopilot, Mode: invoke.ModeRun, Sandbox: s,
			Approval: invoke.ApprovalAutoAll,
			Config:   map[string]string{"uxp.allow_dangerous": "true"},
		})
		if err == nil {
...
func TestBuildApprovalAutoEditRefused(t *testing.T) {
	t.Parallel()
	_, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCopilot, Mode: invoke.ModeRun, Approval: invoke.ApprovalAutoEdit,
	})
	if err == nil {
...
func TestBuildApprovalAutoAllOptIn(t *testing.T) {
	t.Parallel()
	spec, _, err := New().Build(invoke.Invocation{
		CLI: axon.HostCopilot, Mode: invoke.ModeRun,
		Approval: invoke.ApprovalAutoAll,
		Config:   map[string]string{"uxp.allow_dangerous": "true"},
	})
	if err != nil {
...
func TestBuildModeRunWithoutAutoApproveWarns(t *testing.T) {
	...
	_, ds, err := New().Build(invoke.Invocation{
		CLI: axon.HostCopilot, Mode: invoke.ModeRun, Prompt: "x",
	})
	if err != nil {
		t.Fatalf("err = %v", err)
	}
