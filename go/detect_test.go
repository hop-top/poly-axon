...
func TestDetect_FoundWithVersion(t *testing.T) {
	...
	result, err := Detect(HostClaude, &DetectOpts{
...
func TestDetect_NotInstalled(t *testing.T) {
	...
	result, err := Detect(HostClaude, &DetectOpts{Runner: runner})
	if err != nil {
		t.Fatalf("Detect: %v", err)
	}
	if result.Installed {
...
func TestDetect_NilOpts(t *testing.T) {
	...
	result, err := Detect(HostClaude, nil)
	if err != nil {
		t.Fatalf("Detect with nil opts: %v", err)
	}
	// claude --version may or may not work on the host running the test;
...
func TestDetect_FallbackBinary(t *testing.T) {
	...
	result, err := Detect(HostAntigravity, &DetectOpts{
		Runner: runner, LookPath: lp,
	})
	if err != nil {
		t.Fatalf("Detect: %v", err)
...
func TestDetect_StoreCheckUsesResolveStorePath(t *testing.T) {
	runner := &stubExecRunner{
		binaries: map[string]string{"claude": "v1.0.0\n"},
	}
	lp := &stubLookPather{
	...
	// The error message, if any, should contain an expanded path (with
	// home dir), not the raw "~/" — proving ResolveStorePath was used.
